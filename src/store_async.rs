...
pub async fn run_store_async(
    ...
    let mut association = options
        ...
        .await
        .whatever_context("Could not establish store association")?;
    ...
    let mut last_study_dir = PathBuf::new();
    loop {
                    ...
                    Pdu::PData { ref mut data } => {
                        ...
                        for data_value in data {
                                ...
                                && data_value.is_last
                            {
                                ...
                                // does current instance belong to the same study?
                                let study_dir = match study_uid {
                                    Some(uid) if !last_study_dir.ends_with(&uid) => {
                                        let study_dir = output_dir.join(&uid);
                                        std::fs::create_dir_all(&study_dir).whatever_context(format!("Could not create output study directory at `{study_dir:?}`"))?;
                                        last_study_dir = study_dir;
                                        last_study_dir.clone()
                                    }
                                    _ => last_study_dir.clone(),
                                };
                                ...
                                let file_path = study_dir.join(
                                    sop_instance_uid.trim_end_matches('\0').to_string() + ".dcm",
                                );

                                if verbose && file_path.exists() {
                                    warn!("Overwriting file at {file_path:?}");
                                }
